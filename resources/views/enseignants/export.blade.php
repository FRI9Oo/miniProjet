...
<head>
    <meta charset="utf-8">
    <title>Planning — {{ $enseignant->nom }} {{ $enseignant->prenom }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .summary { display: flex; justify-content: center; gap: 20px; margin-bottom: 25px; }
        .summary-box { text-align: center; padding: 10px 20px; background: #f7f8fa; border-radius: 8px; }
        .summary-box strong { display: block; font-size: 20px; color: #e94560; }
        .summary-box span { font-size: 11px; color: #8892a4; }
...
<body>
    ...
    <div class="header">
        <h1>Planning — {{ $enseignant->nom }} {{ $enseignant->prenom }}</h1>
        <p>{{ $enseignant->specialite ?? 'Enseignant' }} | Généré le {{ date('d/m/Y') }}</p>
    </div>
                            ...
                            @if($cours)
                                <div class="cours-cell">
                                    ...
                                    <div class="cours-meta">
                                        {{ $cours->filiere->code }} — {{ $cours->salle->code }}
                                    </div>
                                </div>
                            @endif
                        </td>
